...
def FindInputModels(tasdir, zosdir, scenario):
    ...
    if scenario_test:
        ...
        (include_models, include_scenarios) = tas_limit_filter(
            tasdir, temp_target, temp_target_window
        )
    ...
    # Simply return a list of zoz models available for the selected scenario
        ...
        for this_model in os.listdir(zosdir):
            ...
            model_has_scenario = fnmatch.filter(
                os.listdir(os.path.join(zosdir, this_model)), "*{}*".format(scenario)
            )
...
def tas_limit_filter(
    tasdir,
    temp_target,
    temp_target_window=0.25,
    ref_syear=1850,
    ref_eyear=1900,
    extrap_eyear=2099,
):
    ...
    for this_modeldir in modeldirs:
        ...
        hist_filename = fnmatch.filter(
            os.listdir(os.path.join(tasdir, this_modeldir)), "*historical*"
        )[0]
        try:
        ...
        ref_idx = np.flatnonzero(
            np.logical_and(hist_years >= ref_syear, hist_years <= ref_eyear)
        )
        ...
        for this_filename in fnmatch.filter(
            os.listdir(os.path.join(tasdir, this_modeldir)), "*ssp*"
        ):
            ...
            stop_extrap_idx = np.flatnonzero(
                np.logical_and(
                    proj_years >= (extrap_eyear - 19), proj_years <= extrap_eyear
                )
            )
            start_extrap_idx = np.flatnonzero(
                np.logical_and(
                    proj_years >= (extrap_eyear - 20), proj_years <= (extrap_eyear - 1)
                )
            )
            ...
            tas_mean_2100 = (
                stop_extrap + (2100 - extrap_eyear + 9) * tas_rate
            ) - ref_tas
            ...
            if (
                tas_mean_2100 >= temp_target - temp_target_window
                and tas_mean_2100 <= temp_target + temp_target_window
            ):
...
def tlm_preprocess_oceandynamics(
    scenario,
    modeldir,
    driftcorr,
    no_correlation,
    ...
    (include_models, include_scenarios) = FindInputModels(
        tasdir, zos_modeldir, scenario
    )

    if not include_models:
        raise Exception(
            "No models found for this scenario or temperature target - {}".format(
                scenario
            )
        )
    ...
    (zostoga_modellist, zostoga_scenariolist, ZOSTOGA) = IncludeCMIP6Models(
        zostoga_modeldir, "zostoga", datayears, include_models, include_scenarios
    )

    # Center, suture, and smooth ZOSTOGA
    sZOSTOGA = np.nan * ZOSTOGA
    for i in np.arange(0, ZOSTOGA.shape[1]):
        (ZOSTOGA[:, i], sZOSTOGA[:, i]) = SmoothZOSTOGA(
            ZOSTOGA[:, i], datayears, baseyear, smoothwin
        )
    ...
    output = {
        ...
        "scenario": scenario,
        "targyears": targyears,
        "mergeZOSZOSTOGA": mergeZOSZOSTOGA,
        "smoothwin": smoothwin,
        "driftcorr": driftcorr,
        "baseyear": baseyear,
    ...
    (_, focus_site_ids, focus_site_lats, focus_site_lons) = ReadLocationFile(
        locationfile
    )

    # Load the ZOS data
    (zos_modellist, zos_scenariolist, ZOS_raw) = IncludeCMIP6ZOSModels(
        zos_modeldir,
        "zos",
        datayears,
        include_models,
    ...
    sZOS = np.apply_along_axis(
        lambda z, idx: z - z[idx], axis=0, arr=sZOS, idx=baseyear_idx
    )
    sZOSTOGAadj = np.apply_along_axis(
        lambda z, idx: z - z[idx], axis=0, arr=sZOSTOGAadj, idx=baseyear_idx
    )
    ...
    output = {
        ...
        "datayears": datayears,
        "focus_site_ids": focus_site_ids,
        "focus_site_lats": focus_site_lats,
        "focus_site_lons": focus_site_lons,
        "sZOSTOGAadj": sZOSTOGAadj,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="",
        choices=["", "ssp119", "ssp126", "ssp245", "ssp370", "ssp585"],
    )
    parser.add_argument(
        "--model_dir",
    ...
    if (
        ...
        and os.path.isfile(zostogafile)
        and os.path.isfile(zosfile)
    ):
        print(
            "{}, {}, and {} found, skipping OD preprocessing".format(
                configfile, zostogafile, zosfile
            )
        )
    else:
