...
def tlm_project_cmip(nsamps, seed, pipeline_id):
    ...
    for i in np.arange(0, len(targyears)):
        ...
        thermsamps[:, i] = (
            ThermExpScale * temp * ThermExpStd[this_year_ind]
        ) + ThermExpMean[this_year_ind]
    ...
    model_string_pieces = [
        "{0}-{1}".format(zostoga_modellist[x], zostoga_scenariolist[x])
        for x in np.arange(len(zostoga_modellist))
    ]
    ...
    # Assign attributes
    rootgrp.description = (
        "Global SLR contribution from thermal expansion according to IPCC AR6 workflow"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run the projection stage for the IPCC AR6 ocean dynamics workflow",
    ...
    projfile = os.path.join(
        os.path.dirname(__file__), "{}_projections.pkl".format(args.pipeline_id)
    )
